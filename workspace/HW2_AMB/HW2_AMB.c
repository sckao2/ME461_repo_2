...
//AMB Initializing variables given in homework sample code
float printLV1 = 0;
float printLV2 = 0;
float printLV3 = 0;
float printLV4 = 0;
...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer0, LAUNCHPAD_CPU_FREQUENCY, 10000); //AMB timer 0's interrupt function is called every 1ms in addition to %10 being used in the function itself
...
__interrupt void cpu_timer0_isr(void)
{
    ...
    //AMB Copying code from homework pdf
    if (NewLVData == 1) {
        NewLVData = 0;
