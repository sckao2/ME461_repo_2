...
//step 3 create numTimer2calls to track when cpu_timer2_isr is called
...
int16_t ReadSwitches(void)
{
    ...
    if (GpioDataRegs.GPADAT.bit.GPIO4 == 0) {
        returnvar=returnvar|0x1;
    } else {
    }
    //pushbutton 2 GPIO5
    if (GpioDataRegs.GPADAT.bit.GPIO5 == 0) {
        returnvar=returnvar|0x1<<1;
    } else {
    }
    //pushbutton 3 GPIO6
...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer2, LAUNCHPAD_CPU_FREQUENCY, 250000);//set this period to 0.25sec, was 40000

    // Enable CpuTimer Interrupt bit TIE
    //CpuTimer0Regs.TCR.all = 0x4000;//DISABLED FOR LAB 2
    //CpuTimer1Regs.TCR.all = 0x4000;//DISABLED FOR LAB 2
    ...
    {
        if (UARTPrint == 1 ) {
			serial_printf(&SerialA,"Num Timer2:%ld Num SerialRX: %ld\r\n",numTimer2calls,numRXA);//changed to print numTimer2calls
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer2_isr(void)
{
	//incrememnt timer 2 counter
    SetLEDRowsOnOff(rowson);
    numTimer2calls++;
    //rowson=21;//this should make the LEDs alternate rows on and off
    rowson=ReadSwitches();//read values from pushbuttons to determine which rows to toggle on and off!
    UARTPrint=1;
